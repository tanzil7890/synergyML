...
from synergyml.llm.base import (
    BaseClassifierMixin as _BaseClassifierMixin,
    BaseTunableMixin as _BaseTunableMixin,
)
from synergyml.utils import to_numpy as _to_numpy
from synergyml.prompts.templates import (
...
from synergyml.prompts.builders import (
    build_zero_shot_prompt_slc,
    build_zero_shot_prompt_mlc,
    build_few_shot_prompt_slc,
    build_few_shot_prompt_mlc,
)
from synergyml.memory.base import IndexConstructor
from synergyml.memory._sklearn_nn import SklearnMemoryIndex
from synergyml.models._base.vectorizer import BaseVectorizer as _BaseVectorizer
from synergyml.utils import re_naive_json_extractor
import json
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(self, _BaseClassifierMixin):
            raise TypeError(
                "Classifier must be mixed with a synergyml.llm.base.BaseClassifierMixin"
                " class"
            )
...
class BaseTunableClassifier(BaseClassifier):
    def fit(
        ...
        if not isinstance(self, _BaseTunableMixin):
            raise TypeError(
                "Classifier must be mixed with a synergyml.llm.base.BaseTunableMixin class"
            )
        super().fit(X, y)
        self._tune(X, y)
        return self
