...
from synergyml.internal.segmentation_utils import _check_input, _get_valid_optional_params
from synergyml.segmentation.filter import find_label_issues
...
def get_label_quality_scores(
    ...
    * W - Width of each image
    ...
    labels:
      ...
      Refer to documentation for this argument in :py:func:`find_label_issues <synergyml.segmentation.filter.find_label_issues>` for further details.
    ...
    method: {"softmin", "num_pixel_issues"}, default="softmin"
      ...
      - "num_pixel_issues" - Uses the number of pixels with label issues for each image using :py:func:`find_label_issues <synergyml.segmentation.filter.find_label_issues>`
...
def issues_from_scores(
    ...
    """
    Converts scores output by `~synergyml.segmentation.rank.get_label_quality_scores`
    to a list of issues of similar format as output by :py:func:`segmentation.filter.find_label_issues <synergyml.segmentation.filter.find_label_issues>`.
    ...
    Note
    ----
    - This method is intended for converting the most severely mislabeled examples into a format compatible with ``summary`` methods like :py:func:`segmentation.summary.display_issues <synergyml.segmentation.summary.display_issues>`.
    - This method does not estimate the number of label errors since the `threshold` is arbitrary, for that instead use :py:func:`segmentation.filter.find_label_issues <synergyml.segmentation.filter.find_label_issues>`, which estimates the label errors via Confident Learning rather than score thresholding.

    Parameters
    ----------
    image_scores:
      Array of shape `(N, )` of overall image scores, where `N` is the number of images in the dataset.
      Same format as the `image_scores` returned by `~synergyml.segmentation.rank.get_label_quality_scores`.

    pixel_scores:
      Optional array of shape ``(N,H,W)`` of scores between 0 and 1, one per pixel in the dataset.
      Same format as the `pixel_scores` returned by `~synergyml.segmentation.rank.get_label_quality_scores`.
    ...
    issues:
      ...
      Use :py:func:`segmentation.summary.display_issues <synergyml.segmentation.summary.display_issues>`
...
def _get_label_quality_per_image(pixel_scores, method=None, temperature=0.1):
    from synergyml.internal.multilabel_scorer import softmin
